...
double const XStop  =  10.0;



double R (double const ix, double const iy, double const iz, double const ox, double const oy, double const oz)
{
  return sqrt( (ox - ix) * (ox - ix) + (oy - iy) * (oy - iy) + (oz - iz) * (oz - iz) );
}
...
int RK4Test ()
{
  ...
  int const N = 6;
  double x[N] = {0.0, 0.00001 * kC_SI, 0.0, 0.00001 * kC_SI, (double) XStart, (double) (BetaZ * kC_SI)};
  double dxdt[N];
...
int main (int argc, char* argv[])
{
  ...
  TBF = new TBFieldUniformB(0, 0, 100);
