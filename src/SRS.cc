...
#ifdef CUDA
__global__ void PowerDensityGPU (double *x, double *y, double *z, double *bx, double *by, double *bz, double *aocx, double *aocy, double *aocz, double *sx, double *sy, double *sz, double *snx, double *sny, double *snz, double *dt, int *nt, int *ns, double *power_density)
{
...
void SRS::CalculatePowerDensityGPU (TParticleA& Particle, TSurfacePoints const& Surface, T3DScalarContainer& PowerDensityContainer, int const Dimension, bool const Directional, std::string const& OutFileName)
{
...
#ifndef CUDA
  return CalculatePowerDensity (Particle, Surface, PowerDensityContainer, Dimension, Directional, OutFileName);
#endif
  ...
  PowerDensityGPU<<<NBlocks, NThreadsPerBlock>>>(d_x, d_y, d_z, d_bx, d_by, d_bz, d_aocx, d_aocy, d_aocz, d_sx, d_sy, d_sz, d_snx, d_sny, d_snz, d_dt, d_nt, d_ns, d_power_density);
  ...
  int    nt = NTPoints;
  int    ns = NSPoints;
  PowerDensityGPU_Single(x, y, z, bx, by, bz, aocx, aocy, aocz, sx, sy, sz, snx, sny, snz, &dt, &nt, &ns, power_density);
#endif
